 ...
 */
package org.springblade.boot.core.secure.utils;
...
public class SecureUtil {
	...
	public static BladeUser getUser(HttpServletRequest request) {
		...
		String clientId = Func.toStr(claims.get(org.springblade.core.secure.utils.SecureUtil.CLIENT_ID));
		Long userId = Func.toLong(claims.get(org.springblade.core.secure.utils.SecureUtil.USER_ID));
		String tenantId = Func.toStr(claims.get(org.springblade.core.secure.utils.SecureUtil.TENANT_ID));
		String roleId = Func.toStr(claims.get(org.springblade.core.secure.utils.SecureUtil.ROLE_ID));
		String account = Func.toStr(claims.get(org.springblade.core.secure.utils.SecureUtil.ACCOUNT));
		String roleName = Func.toStr(claims.get(org.springblade.core.secure.utils.SecureUtil.ROLE_NAME));
		String userName = Func.toStr(claims.get(org.springblade.core.secure.utils.SecureUtil.USER_NAME));
	...
	public static Claims getClaims(HttpServletRequest request) {
		String auth = request.getHeader(org.springblade.core.secure.utils.SecureUtil.HEADER);
		if (StringUtil.isNotBlank(auth) && auth.length() > AUTH_LENGTH) {
			String headStr = auth.substring(0, 6).toLowerCase();
			if (headStr.compareTo(org.springblade.core.secure.utils.SecureUtil.BEARER) == 0) {
				auth = auth.substring(7);
				return org.springblade.core.secure.utils.SecureUtil.parseJWT(auth);
			}
		} else {
			String parameter = request.getParameter(org.springblade.core.secure.utils.SecureUtil.HEADER);
			if (StringUtil.isNotBlank(parameter)) {
				return org.springblade.core.secure.utils.SecureUtil.parseJWT(parameter);
			}
		}
		return null;
	}
